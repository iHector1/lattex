...
% Tarjeta de voltaje con encabezado amarillo y valor en cuerpo gris
\newcommand{\BatteryVoltageCard}[2]{%
  \begin{tikzpicture}
    \pgfmathsetlengthmacro{\VW}{\linewidth}
    \pgfmathsetlengthmacro{\VH}{16mm}
    \pgfmathsetlengthmacro{\HH}{5.2mm}
    \pgfmathsetlengthmacro{\VR}{1.8mm}

    \path[draw=black!15, fill=gray!15, rounded corners=\VR, line width=0.5pt]
      (0,0) rectangle (\VW,\VH);

    \begin{scope}
      \clip[rounded corners=\VR] (0,0) rectangle (\VW,\VH);
      \path[draw=none, fill=warningyellow] (0,\VH-\HH) rectangle (\VW,\VH);
    \end{scope}

    \node[anchor=center, text=black, font=\fontsize{7}{8.2}\selectfont\bfseries]
      at (0.5*\VW,\VH-0.5*\HH) {#1};
    \node[anchor=center, text=black, font=\fontsize{10.5}{11.5}\selectfont]
      at (0.5*\VW,0.5*(\VH-\HH)) {#2};
  \end{tikzpicture}%
}
